...
API_USER = config.get("DEV", "USER")


#conn_string = f"postgresql://{API_USER}:{API_KEY}@localhost:5432/weather_data"
conn_string = f"postgresql://{API_USER}:{API_KEY}@127.0.0.1/weather_data"
#conn_string = f"postgresql://{API_USER}:{API_KEY}@localhost/weather_data"
...
#vi behöver bygga ihop parameterchoice och create_harmony_dict för koden till dag måste gå i en linje. den får inte loopa
...
with DAG("raw_data", start_date=datetime(2021, 1, 1),
    schedule_interval=timedelta(minutes=1), catchup=False) as dag:
        ...
        parameterchoice = PythonOperator(
            ...
            python_callable=parameterchoice
        )
